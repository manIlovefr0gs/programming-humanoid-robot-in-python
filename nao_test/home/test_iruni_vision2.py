"""
Test Script für Desktop Webcam mit Objekterkennung
Iruni/DroidCam über USB - inkl. Farb- und Objekterkennung
"""
...
# HSV-Farbbereiche (können angepasst werden!)
COLOR_RANGES = {
    'rot': [
        [(0, 100, 100), (10, 255, 255)],      # Niedriger Rot-Bereich
        [(170, 100, 100), (180, 255, 255)]    # Hoher Rot-Bereich (Rot überspannt 0°)
    ],
...
def get_dominant_color_name(frame: np.ndarray) -> Optional[str]:
    """
    Erkennt die dominanteste Farbe im Frame automatisch
    
    Args:
        frame: BGR-Frame von OpenCV
        
    Returns:
        Name der dominantesten Farbe oder None
    """
    # In HSV konvertieren
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    
    # Für jede Farbe die Anzahl der Pixel zählen
    color_pixels = {}
    ...
    # Farbe mit den meisten Pixeln zurückgeben
    if max(color_pixels.values()) > 100:  # Minimale Schwelle
...
def detect_auto_color(frame: np.ndarray, min_area: int = MIN_OBJECT_AREA) -> Optional[Dict]:
    """
    Erkennt automatisch die dominanteste Farbe und findet darin Objekte
    ...
    Returns:
        Dict mit Objekt-Informationen oder None
    """
    # Dominanteste Farbe erkennen
    dominant_color = get_dominant_color_name(frame)
    
    if dominant_color is None:
        return None
    
    # Objekt in dieser Farbe erkennen
    obj = detect_object(frame, dominant_color, min_area)
...
def main():
    ...
    print("'2' - Nur GRÜN erkennen")
    print("'3' - Nur BLAU erkennen")
    print("'4' - Nur ROT erkennen")
    print("'a' - ALLE Farben erkennen")
    print("'0' - AUTO-Modus: Farbe automatisch erkennen")
    print("'c' - Konturen an/aus")
    print("=================\n")
    
    # Einstellungen
    current_colors = None  # None = Auto-Modus
    auto_mode = True  # Standard: Auto-Modus
    show_contours = True
    frame_count = 0
    ...
    try:
        while True:
            ...
            # Objekte erkennen
            if auto_mode:
                # Auto-Modus: Farbe automatisch erkennen
                detection = detect_auto_color(frame)
                detections = [detection] if detection else []
            else:
                # Manueller Modus: vordefinierte Farben
            ...
            # Modus-Anzeige
            if auto_mode:
                mode_text = "Modus: AUTO (Farbe automatisch erkannt)"
            else:
                mode_text = f"Modus: MANUELL | Farben: {', '.join([c.upper() for c in current_colors])}"
            ...
            if key == ord('q'):
                ...
                auto_mode = True
                print("Modus: AUTO - Farbe wird automatisch erkannt")
            elif key == ord('1'):
                auto_mode = False
                current_colors = ['gelb']
                ...
                auto_mode = False
                current_colors = ['gelb', 'grün', 'blau', 'rot']
                print("Modus: ALLE Farben")
            elif key == ord('c'):
                show_contours = not show_contours
                print(f"Konturen: {'AN' if show_contours else 'AUS'}")
            elif key == ord('u'):
                # Unlock mouse-locked color
                _mouse_state['locked'] = False
                _mouse_state['locked_color'] = None
                print("Mouse color unlocked")
